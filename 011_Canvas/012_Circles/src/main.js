...
function mover(el) {
    ...
    setInterval(() => {
        ...
        for (let i = 0; i < circles.length; i++) {
            if (el.cx.animVal.value + el.rad === circles[i].cx.animVal.value + circles[i].rad || el.cy.animVal.value + el.rad === circles[i].cy.animVal.value + circles[i].rad) {
                el.speedX = -el.speedX;
                el.speedY = -el.speedY;
